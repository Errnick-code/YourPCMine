...
/**
 * Подменяет стандартный ChatScreen когда активна overlay-сессия (/ypmutil textoverlay).
 *
 * Поведение:
 *  - Команды (начинаются с /) — отправляются как обычно.
 *  - Обычный текст — НЕ уходит в чат, идёт как OverlayText нужному игроку.
 *  - Стрелки вверх/вниз — НЕ листают историю команд (там команды оператора).
 *  - В строке ввода отображается подсказка "[→ имяИгрока]".
 */
...
class YpmChatScreen(
    ...
    /** Листаем полную историю — и обычные сообщения и команды.
     *  Overlay-сообщения в историю не добавляются (см. handleChatInput),
     *  поэтому раскрытия оператора не происходит. */
    ...
    override fun handleChatInput(msg: String, addToRecent: Boolean) {
        ...
        // Команды — обычная обработка, в историю добавляем (это не секрет)
        ...
        // Добавляем в историю чата — чтобы стрелка вверх листала предыдущий ввод
        if (addToRecent) minecraft.gui.getChat().addRecentChat(normalized)
    }
}

/** Параметры активной overlay-сессии, хранятся на клиенте пока сессия активна. */
@Environment(EnvType.CLIENT)
...
/**
 * Возвращает параметры тона по последнему символу текста.
 * Результат: (freqStart, freqEnd, durationMs, wave).
 * Рандом ±10 Гц — чтобы одинаковые фразы не звучали идентично.
 *
 * sine  — мягкий звук (эмоции, интонации)
 * square — резкий/тревожный (восклицание)
 */
@Environment(EnvType.CLIENT)
fun toneParamsForText(text: String): ToneParams {
    ...
    return when (last) {
        '?'       -> ToneParams(360.0 + j, 380.0 + j, 900L, "sine")    // вопрос — небольшой подъём
        '!'       -> ToneParams(100.0 + j, 100.0 + j, 900L, "square")  // восклицание — square ровная
        ')'       -> ToneParams(320.0 + j, 520.0 + j, 900L, "sine")    // радость — подъём вверх
        '('       -> ToneParams(390.0 + j, 280.0 + j, 900L, "sine")    // грусть — спад вниз
        '/', '\\' -> ToneParams(320.0 + j, 360.0 + j, 900L, "sine")    // нейтральный подъём
        else      -> ToneParams(330.0 + j, 350.0 + j, 900L, "sine")    // базовая — почти ровная
    }
}

/** Параметры тона, возвращаемые [toneParamsForText]. */
...
/**
 * Воспроизводит тон через javax.sound.sampled.
 *
 * [wave] — форма волны: "sine" | "square" | "sawtooth" | "triangle"
 *  - sine     — мягкий, музыкальный
 *  - square   — резкий, электронный (для восклицаний/тревоги)
 *  - sawtooth — яркий, пилообразный
 *  - triangle — мягче square, но не такой мягкий как sine
 *
 * Огибающая: нарастание первые 30%, спад последние 70%.
 * Запускается в отдельном daemon-потоке, не блокирует игру.
 */
@Environment(EnvType.CLIENT)
fun playWaveTone(
        ...
        try {
            ...
            for (i in 0 until totalSamples) {
                ...
                val raw = when (wave) {
                    ...
                    else       -> sin(phase)  // "sine" и всё остальное
                }
            ...
            dataLine.drain()
            dataLine.close()
        } catch (_: Exception) { /* нет аудиоустройства — тихо игнорируем */ }
